...
export default function ExerciseDetail() {
  ...
  const [aiGuidance, setAiGuidance] = useState<any>([])
  const [aiLoading, setAiLoading] = useState(false)

  const extractJsonArray = (response: string): string[] => {
    try {
  ...
  const Prompt = {
    IDEA: "Give a concise, beginner-friendly description of this exercise, including its main benefits, primary muscles worked, and one safety tip. Return the result as a JSON array of 3 strings."
  };
  // console.log("aiGuidance",aiGuidance)

  const getAiGuidance = async () => {
    if (!exercise) return
    try {
      setAiLoading(true)
      const result = await getGeminiResponse(exercise.name + Prompt.IDEA);

      const parsedArray = extractJsonArray(result); // <== clean & parse
      console.log("parsedArray", parsedArray);

      const aiGuidanceMarkdown = parsedArray
        .map((item, i) => `**${i + 1}.** ${item}`)
        .join("\n\n");

      setAiGuidance(aiGuidanceMarkdown);
      ...
      >
        ...
        <View className='px-6 py-4'>
          ...
          <View className='mt-8 gap-2'>
            <TouchableOpacity
              className={`rounded-xl py-4 items-center ${aiLoading ? "bg-gray-400" : aiGuidance.length > 0  ? "bg-green-500" : "bg-blue-500"}`}
            ...
            >
              {
                aiLoading ? (
                  ...
                  <Text className="text-lg font-bold text-white">
                    {
                      aiGuidance.length > 0 ? "Refresh AI Guidance" : "Get AI Guidance on Form and Technique"
                    }
                  </Text>
                )
              }
